...
sub process_batch{
    ...
    if($self->use_threads){
        $command = "perl paralellrunner.pl --procs ".$self->procs_per_sbatch." --infile ".$self->cmdfile." --outdir ".$self->outdir;
    }
    elsif($self->use_processes){
        $command = "perl mcerunner.pl --procs ".$self->procs_per_sbatch." --infile ".$self->cmdfile." --outdir ".$self->outdir;
    }
